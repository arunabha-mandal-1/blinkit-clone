...
import com.example.blinkitadminclone.models.Admin
    ...
    fun signInWithPhoneAuthCredential(otp: String, userNumber: String, admin: Admin) {
        ...
        Utils.getAuthInstance().signInWithCredential(credential)
            .addOnCompleteListener { task ->
                admin.uid = Utils.getUserId()
                if (task.isSuccessful) {
                    _isSignedInSuccessfully.value = true

//                    _isCurrentUser.value = true // not here

                    // On successful sign in, store basic admin information
                    Utils.getDatabaseRef().child("AllUsers").child("Admins").child("AdminsInfo").child(admin.uid!!).setValue(admin)
                } else {
                    // ...
                }
            }
